...
public class ComputeCallable<I extends WritableComparable, V extends Writable,
  ...
  public Collection<PartitionStats> call() {
	...
	if (TERAHEAP_ENABLE.get(configuration)) {
		// Move the objects (messages or edges) from the previous superstep to the
		// TeraHeap
		_UNSAFE.moveObjectsToTeraHeap(graphState.getSuperstep());
		// Set the non promote tag
		_UNSAFE.setNonPromoteTag(graphState.getSuperstep() + 1);
	}
