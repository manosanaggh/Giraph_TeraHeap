...
public class LongByteArrayMessageStore<M extends Writable>
  ...
  private DataInputOutput getDataInputOutput(
    Long2ObjectOpenHashMap<DataInputOutput> partitionMap, long vertexId, int partitionId) {
    ...
    if (dataInputOutput == null) {
	  ...
	  if (TERAHEAP_ENABLE.get(config))
		  _UNSAFE.lazyPromoteToTeraHeap(dataInputOutput, 1, partitionId);
    }
      ...
      if (vertexIdMessageBytesIterator != null) {
        while (vertexIdMessageBytesIterator.hasNext()) {
          ...
          DataInputOutput dataInputOutput = getDataInputOutput(partitionMap,
              vertexIdMessageBytesIterator.getCurrentVertexId().get(), partitionId);
        ...
        try {
          ...
          while (iterator.hasNext()) {
            iterator.next();
            DataInputOutput dataInputOutput = getDataInputOutput(partitionMap,
                iterator.getCurrentVertexId().get(), partitionId);
